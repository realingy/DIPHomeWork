...
int main()
{
	...
	warpPerspective(left, imageTransform1, homo, Size(MAX(corners.right_top.x, corners.right_bottom.x), right.rows)); //透视变换
	//warpPerspective(left, imageTransform2, adjustMat*homo, Size(b.cols*1.3, b.rows*1.8));
	imshow("旋转效果", imageTransform1);
//	imwrite("sift_trans.jpg", imageTransform1);

	//创建拼接后的图,需提前计算图的大小
	int dst_width = imageTransform1.cols + right.cols;  //取最右点的长度为拼接图的长度
	...
	////////////////////////////////////////////////////////////////////////
	Mat roi = dst(Rect(0, 0, imageTransform1.cols, imageTransform1.rows));
	//分别令像素值为1、0，以及令mask=img2.clone();观察输出结果
	Mat mask(roi.rows, roi.cols, roi.depth(), Scalar(1));
	imageTransform1.copyTo(roi, mask);

	roi = dst(Rect(imageTransform1.cols, 0, right.cols, right.rows));
	//分别令像素值为1、0，以及令mask=img2.clone();观察输出结果
	Mat mask2(roi.rows, roi.cols, roi.depth(), Scalar(1));
	right.copyTo(roi, mask2);
	////////////////////////////////////////////////////////////////////////
