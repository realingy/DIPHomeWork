...
)
{
	...
	// Ptr<SIFT> sift = xfeatures2d::SIFT::create(15000);
	...
	for (unsigned int k=0;k<image_names.size();k++)
	{
		...
		vector<KeyPoint> kp1;
		Mat d1;
		cout << "checking keypoint: " << name << endl;
			
		//orb->detectAndCompute(img1, Mat(), kp1, d1);
		sift->detectAndCompute(img1, Mat(), kp1, d1);
...
void match_features2 (vector<Mat> image_descriptor, vector<vector<DMatch>>& image_matches)
{
	for (unsigned int i = 0; i < image_descriptor.size () - 1; i++)
	{
		...
		cout << "keypoint count:" << matches.size() << endl;
		...
		image_matches.push_back(good_matches);
#endif
	}
}
